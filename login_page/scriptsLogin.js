...
if (userType == 'centru') {
	const additionalInput = document.getElementById('signupPrenume');
	if (additionalInput) {
		additionalInput.parentNode.removeChild(additionalInput);
	}
}
...
function signUp() {
	// Retrieve user input
		...
		var prenume = document.getElementById('signupPrenume').value;
		var email = document.getElementById('signupEmail').value;
		var password = document.getElementById('signupPassword').value;
		window.location.href = 'http://127.0.0.1:5500/sign-up-form/sign-up-form.html?name=' + encodeURIComponent(name) +
...
async function signIn() {
    ...
    var password = document.getElementById('signinPassword').value;
        ...
        try {
            const response = await fetch('http://localhost:8080/donors/authenticate', {
                method: 'POST',
                headers: {
            ...
            const donorProfileUrl = window.location.origin + '/donor_profile/donor_profile.html';
